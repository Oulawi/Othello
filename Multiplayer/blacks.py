...
pygame.init()
pygame.mixer.quit()
pygame.display.set_caption("Othello (blacks)")
    ...
    if turnCount % 2 == 1:
        if pygame.mouse.get_pressed()[0] == 1 and grid[findMouseLoc()[0]][findMouseLoc()[1]] != 1 and grid[findMouseLoc()[0]][findMouseLoc()[1]] != 2:
            ...
            for i in range(playerMove[0] + 1, 8, 1):
                if grid[i][playerMove[1]] == 1 and  i != playerMove[0] + 1:
                    grid[playerMove[0]][playerMove[1]] = 1
                    for u in range(playerMove[0], i, 1):
                        grid[u][playerMove[1]] = 1
                    lines += 1
                    break
                elif grid[i][playerMove[1]] == 2:
                    None
                else:
                    break
            #Check to the left
            ...
            for i in range(playerMove[1] + 1, 8, 1):
                if grid[playerMove[0]][i] == 1 and  i != playerMove[1] + 1:
                    grid[playerMove[0]][playerMove[1]] = 1
                    for u in range(playerMove[1], i, 1):
                        grid[playerMove[0]][u] = 1
                    lines += 1
                    break
                elif grid[playerMove[0]][i] == 2:
                    None
                else:
                    break
            #Check to the top
            ...
            for i in range(playerMove[0] + 1, 8, 1):
                if y == 8:
                    break
                elif grid[i][y] == 1 and i != playerMove[0] + 1:
                    d = playerMove[1] + 1
                    grid[playerMove[0]][playerMove[1]] = 1
